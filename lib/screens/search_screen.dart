...
class SearchScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<MovieData>(builder: (context, value, child) {
      return value.searchDataError == false
          ? Scaffold(
                  ...
                  itemBuilder: (context, index) {
                    return SearchDataTile(
                      voteAverage: value.searchMap['results'][index]
                              ['vote_average']
                          .toString(),
                      title: value.searchMap['results'][index]
                          ['original_title'],
                      description: value.searchMap['results'][index]
                          ['overview'],
                      urlToImage:
                          'https://image.tmdb.org/t/p/w500${value.searchMap['results'][index]['poster_path']}',
                    );
                  }),
            )
